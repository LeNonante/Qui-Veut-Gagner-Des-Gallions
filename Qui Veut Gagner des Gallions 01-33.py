...
class Question(): #Classe question qui crree des objets questions avec intitulé, reponses etc
    ...
    def getQuestion(self): #Récuperer la question
        return self.q

    def getReponses(self):
        return [self.r1,self.r2,self.r3,self.r4] #Récuperer les réponses

    def getBonneReponse(self) : #Récuperer la bonne réponse
        return self.r

def creer_connexion(fichier) : #Utilisée dans la premiere partie du programme, la connexion à la base de donnée
...
def listeElements():#renvoie une liste de 16 objects questions (15 + 1 swipe), puis 4 personnes (appel à un ami)
    ...
    curseur.execute("SELECT COUNT(*) FROM Questions;") #On compte le nombre de question puis genere 16 nb entre 1 et ce nombre
...
liste_proba_amis=[95,90,85,80] #Contient les proba d'avoir la bonne réponse suivant l'ami
random.shuffle(liste_proba_amis) #on mélange
pygame.init()
...
def utilisationAppel(): #Quand il est dispo et on clique dessus
    ...
    valeur_appel-=2 #on passe sa valeur à 2
...
#on cree ce qui contiendra la réponse de l'appel
str_reponseAppel=''
texteReponseAppel=PoliceQuestion.render(str_reponseAppel,1,(249,235,223)) #On creer nos textes qui contiendront la demande et les noms
pos_Reponse_appel=texteReponseAppel.get_rect()
pos_Reponse_appel.y=-500
        ...
        for event in pygame.event.get():   #On parcours la liste de tous les événements reçus
            ...
            if etat!=4 and etat!=5 and etat!=6: #Si pas perdu
                ...
                if etat!=0 : #Si il faut repondre à une question
                    ...
                    if pos_reponse1.topleft[0]<=coordonnées_souris[0]<=pos_reponse1.bottomright[0] and pos_reponse1.topleft[1]<=coordonnées_souris[1]<=pos_reponse1.bottomright[1] and 1 in liste_reponses_dispo: #On selectionne la r1
                        pos_points_questions.centerx=690
                        pos_points_questions.y=pos_reponse1.bottom-20
                        if pygame.mouse.get_pressed()[0]==1:          #SI APPUYE ON VALIDE R1

                            #On masque la réonse de l'appel
                            pos_Reponse_appel.y=-500

                            if valeur_swipe==2 : #Si on a swipe de question
                                if liste_elements[15].getBonneReponse()==1 :
                                    reponseJuste()
                                    valeur_swipe=0
                            else :
                                #Si on a une question non swipée
                                if liste_elements[niveau-1].getBonneReponse()==1 :  #On compare
                                    reponseJuste()
                                else :
                                    etat=4

                    elif pos_reponse2.topleft[0]<=coordonnées_souris[0]<=pos_reponse2.bottomright[0] and pos_reponse2.topleft[1]<=coordonnées_souris[1]<=pos_reponse2.bottomright[1] and 2 in liste_reponses_dispo: #IDEM R2
                        pos_points_questions.centerx=690
                        ...
                        if pygame.mouse.get_pressed()[0]==1:
                            ...
                            if valeur_swipe==2 : #Si on a swipe de question
                                if liste_elements[15].getBonneReponse()==2 :
                                    reponseJuste()
                                    valeur_swipe=0
                            else :
                                #Si on a une question non swipée
                                if liste_elements[niveau-1].getBonneReponse()==2 :  #On compare
                                    reponseJuste()
                                else :
                                    etat=4

                    elif pos_reponse3.topleft[0]<=coordonnées_souris[0]<=pos_reponse3.bottomright[0] and pos_reponse3.topleft[1]<=coordonnées_souris[1]<=pos_reponse3.bottomright[1] and 3 in liste_reponses_dispo: #IDEM R3
                        pos_points_questions.centerx=690
                        ...
                        if pygame.mouse.get_pressed()[0]==1:
                            ...
                            if valeur_swipe==2 : #Si on a swipe de question
                                if liste_elements[15].getBonneReponse()==3 :
                                    reponseJuste()
                                    valeur_swipe=0
                            else :
                                #Si on a une question non swipée
                                if liste_elements[niveau-1].getBonneReponse()==3 :  #On compare
                                    reponseJuste()
                                else :
                                    etat=4

                    elif pos_reponse4.topleft[0]<=coordonnées_souris[0]<=pos_reponse4.bottomright[0] and pos_reponse4.topleft[1]<=coordonnées_souris[1]<=pos_reponse4.bottomright[1] and 4 in liste_reponses_dispo: #IDEM R4
                        pos_points_questions.centerx=690
                        ...
                        if pygame.mouse.get_pressed()[0]==1:
                            ...
                            if valeur_swipe==2 : #Si on a swipe de question
                                if liste_elements[15].getBonneReponse()==4 :
                                    reponseJuste()
                                    valeur_swipe=0
                            else :
                                #Si on a une question non swipée
                                if liste_elements[niveau-1].getBonneReponse()==4 :  #On compare
                                    reponseJuste()
                                else :
                                    etat=4

                    elif pos_perso1.topleft[0]<=coordonnées_souris[0]<=pos_perso1.bottomright[0] and pos_perso1.topleft[1]<=coordonnées_souris[1]<=pos_perso1.bottomright[1] and str_demandePerso1!='': #On selectionne le perso 1
                        pos_points_questions.centerx=569
                        pos_points_questions.y=pos_perso1.bottom-20
                        if pygame.mouse.get_pressed()[0]==1: #SI APPUYE ON VALIDE P1
                            proba=liste_proba_amis[0] #On recup sa proba parmi les 4
                            aleat=randint(1,100)#On genere un nombre en tre 1 et 100 et verifie si il est dans la proba
                            if aleat <=proba :
                                aide=liste_elements[niveau-1].getBonneReponse()
                            else :
                                aide=randint(1,4)
                                while aide ==liste_elements[niveau-1].getBonneReponse():
                                    aide=randint(1,4)

                            pos_demande_appel.y=-500
                            pos_perso1.y=-500
                            pos_perso2.y=-500
                            pos_perso3.y=-500
                            pos_perso4.y=-500

                            str_reponseAppel="''Humm, je pense que c'est la reponse "+str(aide)+"''"
                            texteReponseAppel=PoliceQuestion.render(str_reponseAppel,1,(249,235,223)) #On creer nos textes qui contiendront la demande et les noms
                            pos_Reponse_appel=texteReponseAppel.get_rect()
                            pos_Reponse_appel.y=625
                            pos_Reponse_appel.centerx=569

                    elif pos_perso2.topleft[0]<=coordonnées_souris[0]<=pos_perso2.bottomright[0] and pos_perso2.topleft[1]<=coordonnées_souris[1]<=pos_perso2.bottomright[1] and str_demandePerso2!='': #IDEM P2
                        pos_points_questions.centerx=569
                        pos_points_questions.y=pos_perso2.bottom-20
                        if pygame.mouse.get_pressed()[0]==1:
                            proba=liste_proba_amis[1]
                            aleat=randint(1,100)#On genere un nombre en tre 1 et 100 et verifie si il est dans la proba
                            if aleat <=proba :
                                aide=liste_elements[niveau-1].getBonneReponse()
                            else :
                    ...
                    elif pos_perso3.topleft[0]<=coordonnées_souris[0]<=pos_perso3.bottomright[0] and pos_perso3.topleft[1]<=coordonnées_souris[1]<=pos_perso3.bottomright[1] and str_demandePerso3!='': #IDEM P3
                        pos_points_questions.centerx=569
                        pos_points_questions.y=pos_perso3.bottom-20
                        if pygame.mouse.get_pressed()[0]==1:
                            proba=liste_proba_amis[2]
                            aleat=randint(1,100)#On genere un nombre en tre 1 et 100 et verifie si il est dans la proba
                            if aleat <=proba :
                                aide=liste_elements[niveau-1].getBonneReponse()
                            else :
                    ...
                    elif pos_perso4.topleft[0]<=coordonnées_souris[0]<=pos_perso4.bottomright[0] and pos_perso4.topleft[1]<=coordonnées_souris[1]<=pos_perso4.bottomright[1] and str_demandePerso4!='': #IDEM P4
                        pos_points_questions.centerx=569
                        pos_points_questions.y=pos_perso4.bottom-20
                        if pygame.mouse.get_pressed()[0]==1:
                            proba=liste_proba_amis[3]
                            aleat=randint(1,100)#On genere un nombre en tre 1 et 100 et verifie si il est dans la proba
                            if aleat <=proba :
                                aide=liste_elements[niveau-1].getBonneReponse()
                            else :
                ...
                if event.type == MOUSEBUTTONDOWN: #Si Clique
                    #print(event.pos[0],event.pos[1])
                    if 144<=event.pos[0]<=191 and 899<=event.pos[1]<=913: #Si clique sur AIDE
                        webbrowser.open("https://docs.google.com/document/d/1fUTm7FHr0uzaEkDh8mrpFgPOC14TUlvQGgj3ZLEau6U/edit?usp=sharing")  #Ouvre Google Doc
                ...
                fenetre.blit(texteReponseAppel,pos_Reponse_appel)
